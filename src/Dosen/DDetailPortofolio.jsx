...
const DDetailPortofolio = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Cari user dari koleksi "users" berdasarkan 10 karakter awal email === nim
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-6">
      ...
      <button
        className="text-purple-700 mb-6 font-medium"
        onClick={() => navigate(-1)}
      >
        ← Kembali
      </button>
      ...
      {/* Konten utama */}
      <div className="max-w-xl mx-auto p-6 bg-white rounded-md shadow-md border border-purple-200 text-center">
        {/* Avatar */}
          ...
          {data.proyek?.map((p, idx) => (
            <div key={idx} className="mb-4">
              <h4 className="font-bold">
                {p.judul} <span className="text-gray-500">({p.tahun})</span>
              </h4>
